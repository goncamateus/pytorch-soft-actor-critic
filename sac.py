...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.critic = QNetwork(
            num_inputs, action_space.shape[1], args.hidden_size).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args.lr)

        self.critic_target = QNetwork(
            num_inputs, action_space.shape[1], args.hidden_size).to(self.device)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(
                num_inputs, action_space.shape[1], args.hidden_size, action_space).to(self.device)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        if type(memory) is ReplayMemory:
            state_batch, action_batch, reward_batch,\
                next_state_batch, mask_batch = memory.sample(
                    batch_size=batch_size)

        elif type(memory) is ReplayGMemory:
            state_batch, action_batch, reward_batch,\
                next_state_batch, mask_batch, goal_batch = memory.sample(
                    batch_size=batch_size)
            state_batch = np.concatenate([state_batch, goal_batch], axis=1)
            next_state_batch = np.concatenate([next_state_batch, goal_batch],
                                              axis=1)
